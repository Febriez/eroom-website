...
export default function NotificationList({
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg overflow-hidden max-h-[500px] flex flex-col" style={{ width: '320px' }}>
        ...
        {filteredNotifications.length === 0 ? (
          ...
          <ul className="divide-y">
            {filteredNotifications.map((notification) => (
              <li key={notification.id} className={notification.read ? '' : 'bg-blue-50'} onClick={() => onMarkAsRead(notification.id)}>
                <Link href={notification.actionUrl || '#'} className="block p-3 hover:bg-gray-50">
                  <div className="flex">
                    <div className={`w-10 h-10 rounded-full ${getNotificationColor(notification.type)} flex items-center justify-center mr-3`}>
